...
export const formatUrl = (value, baseUrl, nullable) => {
    if (nullable && !value) {
...
const displayFromBase = (url, baseUrl) => {
...
export const getYRangeWithLargePadding = (data) => {
    if (!data.length) {
        return { min: 0, max: 1 };
    }
...
export const calculateYAxisWidth = (ticks, formatter) => {
    if (!ticks.length) {
        return 40;
    }
...
// Get range for date
export const getRangeForStartDate = (startDate) => {
 ...
 */
export const sanitizeChartData = (data, range, fieldName = 'value', aggregationType = 'avg') => {
    if (!data.length) {
        return [];
    }
 ...
 */
export const formatDisplayDateWithRange = (date, range) => {
    if (range > 365) {
