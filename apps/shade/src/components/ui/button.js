...
const buttonVariants = cva('inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 [&_svg]:stroke-[1.5px]', {
    variants: {
        variant: {
...
const Button = React.forwardRef(({ className, variant, size, asChild = false, children, ...props }, ref) => {
    ...
    const content = variant === 'dropdown' ? (_jsxs(_Fragment, { children: [children, _jsx(ChevronDown, { className: "!-ml-1 !-mr-0.5 size-4 !stroke-[2px] opacity-50", strokeWidth: 2 })] })) : children;
    return (_jsx(Comp, { ref: ref, className: cn(buttonVariants({ variant, size, className })), ...props, children: content }));
});
