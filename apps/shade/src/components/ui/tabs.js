...
const Tabs = React.forwardRef(({ variant = 'segmented', ...props }, ref) => (_jsx(TabsVariantContext.Provider, { value: variant, children: _jsx(TabsPrimitive.Root, { ref: ref, ...props }) })));
...
const TabsList = React.forwardRef(({ className, ...props }, ref) => {
    ...
    return (_jsx(TabsPrimitive.List, { ref: ref, className: cn(tabsListVariants({ variant, className })), ...props }));
});
TabsList.displayName = TabsPrimitive.List.displayName;
const tabsTriggerVariants = cva('inline-flex items-center justify-center whitespace-nowrap px-3 py-1 ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground [&_svg]:size-4 [&_svg]:shrink-0', {
    variants: {
        variant: {
...
const TabsTriggerCount = ({ className = '', children }) => {
    return (_jsx("span", { className: `ml-1.5 mt-px flex h-5 items-center justify-center rounded-full bg-gray-200 px-1.5 py-0 text-xs font-semibold leading-[21px] text-gray-800 dark:bg-gray-900 dark:text-gray-300 ${className}`, children: children }));
};
...
const TabsContent = React.forwardRef(({ className, ...props }, ref) => {
    const variant = React.useContext(TabsVariantContext);
    return (_jsx(TabsPrimitive.Content, { ref: ref, className: cn(tabsContentVariants({ variant, className })), ...props }));
});
TabsContent.displayName = TabsPrimitive.Content.displayName;
const KpiTabTrigger = ({ children, ...props }) => {
    return (_jsx(TabsTrigger, { className: 'h-auto', ...props, children: children }));
};
const KpiTabValue = ({ color, icon: iconName, label, value, diffDirection, diffValue, className }) => {
    const IconComponent = iconName ? LucideIcons[iconName] : null;
    const diffContainerClassName = cn('flex items-center gap-1 text-xs h-[22px] px-1.5 rounded-sm group/diff cursor-default mt-0.5', diffDirection === 'up' && 'text-green-600 bg-green/10', diffDirection === 'down' && 'text-red-600 bg-red/10', diffDirection === 'same' && 'text-gray-700 bg-muted');
    return (_jsxs("div", { className: cn('group flex w-full flex-col items-start gap-2', className), children: [_jsxs("div", { className: 'flex h-[22px] items-center gap-1.5 text-base font-medium text-muted-foreground transition-all group-hover:text-foreground', "data-type": "value", children: [color && _jsx("div", { className: 'ml-1 size-2 rounded-full opacity-50', style: { backgroundColor: color } }), IconComponent && _jsx(IconComponent, { size: 16, strokeWidth: 1.5 }), label] }), _jsxs("div", { className: 'flex flex-col items-start gap-2 xl:flex-row xl:gap-3', children: [_jsx("div", { className: 'text-[2.3rem] font-semibold leading-none tracking-tighter xl:text-[2.6rem]', children: value }), diffDirection && diffDirection !== 'hidden' &&
                        _jsx(_Fragment, { children: _jsxs("div", { className: diffContainerClassName, children: [_jsx("span", { className: 'font-medium leading-none', children: diffValue }), diffDirection === 'up' &&
                                        ...
                                        _jsx(TrendingDown, { className: '!size-[12px]', size: 14, strokeWidth: 2 })] }) })] })] }));
};
const KpiDropdownButton = React.forwardRef(({ variant = 'dropdown', className, ...props }, ref) => {
    return (_jsx(Button, { ref: ref, className: cn('h-auto w-full rounded-none border-x-0 border-t-0 focus-visible:ring-0 bg-transparent py-5', className), variant: variant, ...props }));
});
