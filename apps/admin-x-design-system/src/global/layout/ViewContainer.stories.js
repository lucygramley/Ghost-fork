import { jsx as _jsx } from "react/jsx-runtime";
import { useArgs } from '@storybook/preview-api';
import ViewContainer from './ViewContainer';
import Button from '../Button';
import ButtonGroup from '../ButtonGroup';
const meta = {
    title: 'Global / Layout / View Container',
    component: ViewContainer,
    parameters: {
        layout: 'fullscreen'
    },
    render: function Component(args) {
        const [, updateArgs] = useArgs();
        return _jsx(ViewContainer, { ...args, onTabChange: (tab) => {
                updateArgs({ selectedTab: tab });
                args.onTabChange?.(tab);
            } });
    },
    argTypes: {
        children: {
            control: {
                type: 'text'
            }
        }
    },
    tags: ['autodocs'],
    excludeStories: ['exampleActions']
};
export default meta;
export const exampleActions = [
    _jsx(Button, { label: 'Filter', outlineOnMobile: true, onClick: () => {
            alert('Clicked filter');
        } }),
    _jsx(Button, { label: 'Sort', outlineOnMobile: true, onClick: () => {
            alert('Clicked sort');
        } }),
    _jsx(Button, { icon: 'magnifying-glass', iconSize: 'sm', outlineOnMobile: true, onClick: () => {
            alert('Clicked search');
        } }),
    _jsx(ButtonGroup, { buttons: [
            {
                icon: 'listview',
                size: 'sm',
                iconColorClass: 'text-black',
                onClick: () => {
                    alert('Clicked list view');
                }
            },
            {
                icon: 'cardview',
                size: 'sm',
                iconColorClass: 'text-grey-500',
                onClick: () => {
                    alert('Clicked card view');
                }
            }
        ], clearBg: false, link: true, outlineOnMobile: true })
];
const primaryAction = {
    title: 'Add item',
    color: 'black',
    onClick: () => {
        alert('Clicked primary action');
    }
};
const tabs = [
    {
        id: 'steph',
        title: 'Steph Curry',
        contents: 'The tabs component lets you add various datasets. It uses the <code>`TabList`</code> component to stay consistent with the simple TabView.'
    },
    {
        id: 'klay',
        title: 'Klay Thompson',
        contents: 'Splash brother #11.'
    }
];
export const Default = {
    args: {
        type: 'page',
        toolbarBorder: false,
        children: 'The view container component is the main container of pages and/or sections on a page. Select one of the stories on the right to browse use cases.'
    }
};
export const PageType = {
    name: 'Type: Page',
    args: {
        type: 'page',
        title: 'Page title',
        children: 'In its simplest form you can use this component as the main container of pages.'
    }
};
export const SectionType = {
    name: 'Type: Section',
    args: {
        type: 'section',
        title: 'Section title',
        children: 'This example shows how to use it for sections on a page.'
    }
};
export const PrimaryActionOnPage = {
    args: {
        type: 'page',
        title: 'Page title',
        primaryAction: primaryAction,
        children: 'View contents'
    }
};
export const ActionsOnPage = {
    args: {
        type: 'page',
        title: 'Page title',
        actions: exampleActions,
        primaryAction: primaryAction
    }
};
export const PrimaryActionOnSection = {
    args: {
        type: 'section',
        title: 'Section title',
        primaryAction: primaryAction
    }
};
export const MultipleTabs = {
    args: {
        type: 'section',
        title: 'Section title',
        tabs: tabs
    }
};
export const TabsWithPrimaryAction = {
    args: {
        type: 'section',
        title: 'Section title',
        tabs: tabs,
        primaryAction: primaryAction
    }
};
const sectionActions = [
    _jsx(Button, { label: 'Filter', size: 'sm', onClick: () => {
            alert('Clicked filter');
        } }),
    _jsx(ButtonGroup, { buttons: [
            {
                icon: 'listview',
                size: 'sm',
                iconColorClass: 'text-black',
                onClick: () => {
                    alert('Clicked list view');
                }
            },
            {
                icon: 'cardview',
                size: 'sm',
                iconColorClass: 'text-grey-500',
                onClick: () => {
                    alert('Clicked card view');
                }
            }
        ], clearBg: false, size: 'sm', link: true })
];
export const TabsWithActions = {
    args: {
        type: 'section',
        title: 'Section title',
        tabs: tabs,
        primaryAction: primaryAction,
        actions: sectionActions
    }
};
export const HiddenActions = {
    args: {
        type: 'section',
        title: 'Hover to show actions',
        tabs: tabs,
        actions: sectionActions,
        actionsHidden: true
    }
};
