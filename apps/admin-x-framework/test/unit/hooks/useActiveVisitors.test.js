...
describe('useActiveVisitors', () => {
    ...
    beforeEach(() => {
        ...
        mockGetStatEndpointUrl.mockImplementation((_config, endpoint) => `https://api.example.com/${endpoint}`);
    ...
    it('handles statsConfig changes correctly', () => {
        ...
        const { rerender } = renderHook(({ statsConfig }) => useActiveVisitors({ statsConfig, enabled: true }), { initialProps: { statsConfig: initialStatsConfig }, wrapper });
    ...
    it('does not update lastKnownCount when disabled', () => {
        ...
        const { result, rerender } = renderHook(({ enabled }) => useActiveVisitors({ enabled }), { initialProps: { enabled: true }, wrapper });
    ...
    it('resets interval when enabled state changes', () => {
        ...
        const { rerender } = renderHook(({ enabled }) => useActiveVisitors({ enabled }), { initialProps: { enabled: true }, wrapper });
