...
const createWrapper = (sentryDSN) => {
    const TestWrapper = ({ children }) => (_jsx(FrameworkProvider, { externalNavigate: () => { }, ghostVersion: '5.x', sentryDSN: sentryDSN || '', unsplashConfig: {
            Authorization: '',
            'Accept-Version': '',
            'Content-Type': '',
            'App-Pragma': '',
...
describe('useHandleError', () => {
    beforeEach(() => {
        ...
        showToast.mockImplementation(mockShowToast);
        toast.remove.mockImplementation(mockToastRemove);
