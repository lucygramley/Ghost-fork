...
const wrapper = ({ children }) => (_jsx(FrameworkProvider, { externalNavigate: () => { }, ghostVersion: '5.x', sentryDSN: '', unsplashConfig: {
    ...
    }, onDelete: () => { }, onInvalidate: () => { }, onUpdate: () => { }, children: _jsx(QueryClientProvider, { client: queryClient, children: children }) }));
describe('API hooks', () => {
        ...
        it('can transform return data', async () => {
            await withMockFetch({ json: { test: 1 } }, async () => {
                const useTestQuery = createPaginatedQuery({
                    dataType: 'test',
                    path: '/test/',
                    returnData: data => ({ test: data.test + 1, meta: undefined })
                });
                const { result } = renderHook(() => useTestQuery(), { wrapper });
                await waitFor(() => expect(result.current.isLoading).toBe(false));
            ...
            await withMockFetch({
                ...
                const useTestQuery = createInfiniteQuery({
                    ...
                    defaultNextPageParams: (lastPage, otherParams) => ({
                        ...
                        page: (lastPage.pagination.next || 1).toString()
                    }),
                ...
                const useTestMutation = createMutation({
                    ...
                    updateQueries: {
                        ...
                        update: (newData, currentData) => {
                            return { test: newData.test + currentData.test };
                        }
                    }
                });
                const { result } = renderHook(() => useTestMutation(), { wrapper });
