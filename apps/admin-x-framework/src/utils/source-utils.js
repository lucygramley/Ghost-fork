...
export const SOURCE_DOMAIN_MAP = {
    Reddit: 'reddit.com',
...
export const SOURCE_NORMALIZATION_MAP = new Map([
...
export function extendSourcesWithPercentages({ processedData, totalVisitors, mode }) {
    if (mode === 'growth') {
...
export const isDomainOrSubdomain = (sourceDomain, siteDomain) => {
    // Exact match
...
export const getFaviconDomain = (source, siteUrl) => {
...
export function processSources({ data, mode, siteUrl, siteIcon, defaultSourceIconUrl }) {
    if (!data) {
        return [];
    }
