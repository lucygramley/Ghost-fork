 ...
 */
export function createMswServer(additionalHandlers = []) {
 ...
 */
export function setupMswServer(additionalHandlers = []) {
 ...
 */
export function createHandler(method, path, response, status = 200) {
 ...
 */
export function createErrorHandler(method, path, status = 500) {
    const httpMethod = http[method];
    return httpMethod(path, () => {
        return new HttpResponse(null, { status });
    });
