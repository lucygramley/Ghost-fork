...
export const useBrowseActions = createInfiniteQuery({
    dataType,
    path: '/actions/',
    returnData: (originalData) => {
        const { pages } = originalData;
        let actions = pages.flatMap(page => page.actions.map(({ context, ...action }) => ({ ...action, context: JSON.parse(context) })));
...
// Helpers
export const getActorLinkTarget = (action) => {
    if (!action.actor) {
        return;
    }
...
export const getLinkTarget = (action) => {
            ...
            case 'post':
                ...
                if (resourceType === 'post') {
                    if (action.context?.type) {
                        resourceType = action.context?.type;
                    }
                }
                return {
                    isExternal: true,
...
export const getActionTitle = (action) => {
    ...
    if (action.event === 'edited') {
        if (action.context?.action_name) {
            actionName = action.context?.action_name;
        }
    }
    if (action.context?.count && action.context?.count > 1) {
...
export const getContextResource = (action) => {
...
export const isBulkAction = (action) => typeof action.context?.count === 'number' && action.context?.count > 1;
